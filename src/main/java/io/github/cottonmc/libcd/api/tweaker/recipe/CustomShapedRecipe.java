...
public class CustomShapedRecipe extends ShapedRecipe {
	...
	public CustomShapedRecipe(ScriptBridge bridge, Identifier id, String group, int width, int height, DefaultedList<Ingredient> ingredients, ItemStack output) {
	...
	public boolean matches(CraftingInventory inv, World world) {
		...
		try {
			...
			if (result instanceof Boolean) return (Boolean) result;
				...
				logger.error("Could not check match for custom shaped recipe %s, returning standard match: function 'matches' must return a boolean, but returned %s instead", getId(), result.getClass().getName());
				return true;
			}
		} catch (Exception e) {
			logger.error("Could not check match for custom shaped recipe %s, returning standard match: %s", getId(), e.getMessage());
		}
